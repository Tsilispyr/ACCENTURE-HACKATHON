# Vendor Risk & Procurement Deep Agent - Demo & Testing Prompts
...
## 1. 🛡️ Human-In-The-Loop (HITL) Showcase (FR12)
...
* **What Happens**:
  ...
  3. **Chainlit Action Modal appears** with 3 options:
     ...
     - **`Approve with conditions`**: Prompts the user: *"What must be true? One condition per line."* Enter conditions (e.g., `Require SOC 2 Type II report before rollout` and `Vendor must notify incidents within 72 hours`). The pipeline carries these conditions into the final executive report.
...
```
* **Expected UI Stream & Results**:
  - **Planning (`s4_plan`)**: Decomposes the request into security, compliance, commercial, and governance steps.
  - **Hybrid RAG (`s3_ground` & `s6_act`)**: Retrieves citations across `vendor-x-proposal.pdf`, `information-security-policy.pdf`, `procurement-policy.pdf`, and `vendor-x-pricing.pdf`.
  ...
  - **Synthesis (`s8_compose`)**: Recommends **`CONDITIONAL APPROVAL`** or **`REJECT`**, identifying:
    - *Security finding*: Asteria commits to 5-day breach notification, violating Northstar's mandatory 72-hour policy.
    - *Commercial finding*: 3-year TCO calculated at €580,000 (€180k/yr licenses + €40k onboarding).
...
### Security & Incident SLA Contradiction (FR04, FR10)
```text
...
### Commercial & Total Cost of Ownership (MCP Tool Use - FR06)
```text
...
### Historical Precedent & Prior Vendor Approvals (MCP Tool Use - FR06)
```text
...
## 4. 🛑 Guardrails & Prompt Injection Defenses (FR08, FR09)
...
```
* **Expected Result**: Intercepted immediately at **`s2_guard_in`** by dual-layer regex + LLM classifier guardrail. Output:
  > **`Refused.`** *(Request refused by safety guardrails).*
...
```
* **Expected Result**: Distance ceiling rejects all chunks; zero evidence is gathered. The pipeline outputs:
  > **`Refused.`** *The knowledge corpus does not contain information to answer this question.*
